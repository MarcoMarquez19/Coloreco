...
<script lang="ts">
	...
	async function cargarEscena() {
		try {
			...
			if (obraId) {
				// Modo edición: cargar la obra existente
				modoEdicion = true;
				...
				console.log('[ModoDibujo] Cargando obra para edición:', obraEnEdicion.titulo);
				
				// No necesitamos cargar una escena específica en modo edición
				escena = null;
			} else {
				// Modo normal: cargar escena del catálogo
				modoEdicion = false;
				obraEnEdicion = null;
	...
	async function inicializarTaller() {
		...
		// Si estamos en modo edición, cargar la imagen de la obra
		if (modoEdicion && obraEnEdicion?.urlImagen) {
			try {
				...
				mostrarMensajeFeedback('Obra cargada para edición', 'success');
			} catch (error) {
